...
void WIDE_STRING_initialize(WIDE_STRING *pWide_string) {
    pWide_string->pANSI_string = NULL;
...
#ifdef UNICODE
    return pWide_string->pWide_string;
#else
    pWide_string->pANSI_string = STRING_UTILITIES_wide_to_ANSI(pWide_string->pWide_string);
    return pWide_string->pANSI_string;
#endif
}
...
void WIDE_STRING_append_string_a(WIDE_STRING *pOriginal, LPCSTR pAdditional) {
    const size_t characters = wcslen(pOriginal->pWide_string) + strlen(pAdditional) + 1;
    LPWSTR pString = malloc(sizeof(WCHAR) * characters);
    if (!pString) {
        return;
    }
    ...
    wcscpy(pString, pOriginal->pWide_string);
    strcat((char *)pString, pAdditional);
...
void WIDE_STRING_destroy(WIDE_STRING *pWide_string) {
    MEMORY_HELPER_free(pWide_string->pANSI_string);
    MEMORY_HELPER_free(pWide_string->pWide_string);
}
