...
namespace StringUtilities {
    constexpr uint8_t UTF8String::GetFirstUTF8ByteSize(const byte UTF8) noexcept {
    ...
    std::shared_ptr<const CHAR[]> UTF8String::GetWindowsANSIString(void) const noexcept {
        return UTF16ToANSI(UTF8ToUTF16(reinterpret_cast<LPCSTR>(GetBytesWithNullTerminator().data())).get());
    }

    std::shared_ptr<const WCHAR[]> UTF8String::GetWindowsUTF16String(void) const noexcept {
        return UTF8ToUTF16(reinterpret_cast<LPCSTR>(GetBytesWithNullTerminator().data()));
    }

    const std::shared_ptr<const TCHAR[]> UTF8String::GetWindowsTString(void) const noexcept {
#if UNICODE
        return GetWindowsUTF16String();
#else
        return GetWindowsANSIString();
#endif
    }
    ...
    inline std::unique_ptr<const WCHAR[]> ANSIToUTF16(LPCSTR pANSIString) {
        if (!pANSIString) {
            throw std::invalid_argument{ "pANSIString is nullptr." };
        }
    ...
    inline std::unique_ptr<const CHAR[]> UTF16ToANSI(LPCWSTR pUTF16String) {
        if (!pUTF16String) {
            throw std::invalid_argument{ "pUTF16String is nullptr." };
        }
    ...
    inline std::unique_ptr<const byte[]> UTF16ToUTF8(LPCWSTR pUTF16String) {
        ...
        std::unique_ptr<byte[]> UTF8String = std::make_unique<byte[]>(UTF8Characters);

        if (!WideCharToMultiByte(CP_UTF8, 0, pUTF16String, -1, reinterpret_cast<LPSTR>(UTF8String.get()), UTF8Characters, nullptr, nullptr)) {
            STRINGUTILITIES_UTF8STRING_THROW();
        }
        return UTF8String;
    }
    ...
    inline std::unique_ptr<const WCHAR[]> UTF8ToUTF16(LPCSTR pUTF8String) {
        if (!pUTF8String) {
            throw std::invalid_argument{ "pUTF8String is nullptr." };
        }
